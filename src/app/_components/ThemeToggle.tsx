...
export default function ThemeToggle({ className = "", showLabel = false }: { className?: string; showLabel?: boolean }) {
  ...
  return (
    <button onClick={toggle} aria-label={theme === "dark" ? "Cambiar a modo claro" : "Cambiar a modo oscuro"}
      ...
      className={`theme-toggle inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-lg border border-[var(--bd)] text-sm text-[var(--mut)] hover:bg-white/5 hover:text-[var(--tx)] ${showLabel ? "px-3" : "w-9"} ${className}`}>
