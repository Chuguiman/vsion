...
export async function importCartera(rows: any[]): Promise<number> {
  ...
  for (let i = 0; i < marks.length; i += CHUNK) {
    ...
    await db`INSERT INTO client_marks ${db(batch, "case_id", "code", "denom", "classes", "pys", "holder", "attorney", "status")}`;
  }
  return marks.length;
}
...
export async function loadMarksFromDb(): Promise<ClientMark[]> {
  ...
  const rows = await db<{
    ...
    attorney: string | null; status: string | null;
  }[]>`
    SELECT case_id, code, denom, classes, pys, holder, attorney, status FROM client_marks
  `;
  return rows.map((r) => ({
    id: r.case_id ?? "",
    code: r.code ?? "",
